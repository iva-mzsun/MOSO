...
class ReGenerator(object):
    def __init__(self, gen_opt, cut_prob,
                 start_step, tot_steps,
                 init_tokens, init_confidence,
                 mask_token_id, gamma_function):
        ...
        self.cur_confidence = init_confidence # [B, L]
        self.final_tokens = repeat(init_tokens, 'b l -> b n l', n=tot_steps)  # [B, N, L], N is the tot steps

        # Mask tokens with confidence < cut_prob
        self.cnt_mark = init_confidence < cut_prob
        self.cur_tokens[init_confidence < cut_prob] = mask_token_id

    def generate_one_step(self, logits):
        """ Get confidence of cur_tokens and resampled tokens with confidence < self.cut_prob
        ...
        cur_confidence = self.cur_confidence
        ...
        sampled_confidence = torch.gather(probs, -1, sampled_ids.unsqueeze(-1).to(torch.int64)).squeeze(-1)  # [B, L]
        ...
        refresh_ids = sampled_ids * unknown_map + cur_tokens * known_map
        refresh_confidence = sampled_confidence * unknown_map + cur_confidence * known_map

        # update records
        self.final_tokens[:, step, :] = refresh_ids
        self.cur_tokens = refresh_ids
        self.cur_confidence = refresh_confidence
        self.cur_step += 1

        # mask tokens with confidence < cut_prob
        self.cnt_mark = self.cnt_mark + (self.cur_confidence < self.cut_prob)
        self.cur_tokens[self.cur_confidence < self.cut_prob] = self.mask_token_id
        if self.cur_step == self.tot_steps - 1:
            self.cur_tokens[~self.cnt_mark] = self.mask_token_id
    ...
    def unfinished(self):
        """return 0/False as finished, 1/True as unfinished"""
        flag0 = self.cur_step < self.tot_steps
        flag1 = torch.all(self.cur_confidence >= self.cut_prob)
        return flag0 and not flag1
    ...
    def mask_by_topk(self, mask_len, probs):
        ...
        """
        confidence = probs
        sorted_confidence, _ = torch.sort(probs, dim=-1)
...
class ReGenerator_mo(object):
    def __init__(self, num_frames, gen_opt, cut_prob,
                 start_step, tot_steps,
                 init_tokens, init_confidence,
                 mask_token_id, gamma_function):
        """Fast decoding for iterative generation.
        ...
        for i in range(T):
            ...
            cur_init_confidence = init_confidence[:, i*L:(i+1)*L]
            cur_generator = ReGenerator(gen_opt, cut_prob, start_step, tot_steps,
                                        cur_init_tokens, cur_init_confidence, mask_token_id, gamma_function)
    ...
    def unfinished(self):
        ...
        flag = [self.mo_generators[i].unfinished()
                for i in range(self.num_frames)]
        return torch.any(torch.tensor(flag))
