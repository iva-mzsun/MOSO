...
from .splittrain_StackTransformer import StackTransformer_st

from ipdb import set_trace as st

class MoCoTransformer_st(object):
    def __init__(self, train_opt, model_opt, vqvae_opt, tokenizer_opt, generation_opt, load_vqvae):
        ...
        if transformer_type in ['stack']:
            transformer = StackTransformer_st(T=self.tokenizer.num_frames,
                                           model_opt=model_opt,
    ...
    def one_step_forward(self, ybg, yid, ymo, cur_rate):
        hmo = self.tokenizer.get_hmo(ybg.shape[0]).to(self.train_opt.device)

        # Stage I: train bg/id prediction
        xbg, maskbg = self.tokenizer.random_mask(ybg, cur_rate)
        xid, maskid = self.tokenizer.random_mask(yid, cur_rate)
        logits_bg, logits_id, _ = self.model_engine.forward_bgid(xbg, xid, hmo)

        # Stage II: train mo prediction
        xmo, maskmo = self.tokenizer.random_mask_mo(ymo, cur_rate)
        bg_new = self.sample(logits_bg) * maskbg + xbg * (1 - maskbg)
        id_new = self.sample(logits_id) * maskid + xid * (1 - maskid)
        _, _, zmo = self.model_engine.forward_bgid(bg_new, id_new, hmo)
        logits_mo = self.model_engine.forward_mo(zmo, xmo)

        # calculate loss
        target_bg = ybg.reshape(-1)[maskbg.reshape(-1) == 1]
        target_id = yid.reshape(-1)[maskid.reshape(-1) == 1]
        target_mo = ymo.reshape(-1)[maskmo.reshape(-1) == 1]
    ...
    def train_one_step(self, inputs):
        ...
        # Forward
        loss, loss_bg, loss_id, loss_mo = self.one_step_forward(xbg, xid, xmo, cur_rate)
