...
class StackTransformer(nn.Module):
    ...
    def forward_bgid(self, xbg, xid, hmo):
        ## Stage One:
        ...
        return logits_bg, logits_id, stackone_output
    ...
    def forward(self, xbg, xid, hmo, xmo):
        '''
        ...
        logits_bg, logits_id, stackone_output = self.forward_bgid(xbg, xid, hmo)

        if xmo is not None:
            bg_new = self.sample(logits_bg)
            id_new = self.sample(logits_id)
            _, _, stackone_output = self.forward_bgid(bg_new, id_new, hmo)
