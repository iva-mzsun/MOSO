...
class Tokenizer(nn.Module):
    def __init__(self, vqvae_opt, train_opt, tokenizer_opt, load_vqvae):
        ...
        self.pre_frame_num = tokenizer_opt.pre_frame_num
    ...
    def obtain_bg_id_tokens(self, xs):
        # xs: [B T C H W]
        B = xs.shape[0]
        bg_tokens = self.vqvae._encode_bg(xs).reshape(B, -1)
        id_tokens = self.vqvae._encode_id(xs).reshape(B, -1)
    ...
    def random_mask_mo(self, mo_tokens, rate, T=None):
