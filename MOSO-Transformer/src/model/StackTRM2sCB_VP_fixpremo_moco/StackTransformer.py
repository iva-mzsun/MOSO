...
class StackTransformer(nn.Module):
    def __init__(self, model_opt, T, vocab_size):
        ...
        self.encoder_c = nn.Sequential(
            *[Encoder_Layer(model_opt.hidden_dim, model_opt.immediate_hidden_dim,
                            model_opt.encoder_opt, dropout=model_opt.dropout)
    ...
    def forward_bgid(self, xbg, hmo):
        ...
        position_embed = self.pos_embed(torch.cat([xbg, hmo], dim=1))  # [1, bg+id+T, d_model]
        input_bgidhmo = torch.cat([bg_embed, hmo_embed], dim=1) + position_embed
        ...
        return logits_bg, stackone_output

    def forward_mo(self, zmo, xmo):
    ...
    def forward(self, xbg, hmo, xmo):
        ...
        # stage1: predict new bg/id
        logits_bg, stackone_output = self.forward_bgid(xbg, hmo)

        if xmo is not None:
            bg_new = self.sample(logits_bg)
            _, stackone_output = self.forward_bgid(bg_new, hmo)

            ## Stage Two: Predict logits_mo
            zmo = stackone_output[:, xbg.shape[1]:, :]  # [B, T, hidden_dim]
            logits_mo = self.forward_mo(zmo, xmo)
        else:
            logits_mo = None

        return logits_bg, logits_mo
