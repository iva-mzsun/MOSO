...
class MoCoTransformer(object):
    ...
    def one_step_forward(self, bg_tar, mo_tar, bg_pre, mo_base, cur_rate):
        ...
        logits_bg, logits_mo = self.model_engine(bg_pre, hmo, xmo)  # logits: [B, L, vocab_size]
        ...
        loss = ((loss_bg + loss_mo) / 2.0).to(torch.float16)

        return loss, loss_bg, loss_mo
    ...
    def train_one_step(self, inputs):
        ...
        loss, loss_bg, loss_mo = self.one_step_forward(bg_tar, mo_tar, bg_pre, mo_base, cur_rate)
        ...
        if self.cur_step % 10 == 0 and self.train_opt.wandb and dist.get_rank()==0:
            log = {'loss': loss, 'rate': cur_rate,
                   'loss_mo': loss_mo, 'loss_bg': loss_bg,
    ...
    def valid_one_step(self, inputs):
        ...
        return {'loss': loss, 'loss_mo': loss_mo, 'loss_bg': loss_bg, 'loss_id': torch.tensor(0)}
    ...
    def generate_one_sample(self, bg_pre, mo_base, timesteps):
        ...
        with torch.no_grad():
            logits_bg, stackone_output = self.model_engine.forward_bgid(bg_pre, hmo)  # logits: [B, L, vocab_size]
        xbg = self.sample(logits_bg[:, :, self.tokenizer.video_vocab_s:self.tokenizer.cmd_vocab_s]).to(
            self.train_opt.device)
        with torch.no_grad():
            _, _, stackone_output = self.model_engine.forward_bgid(xbg, hmo)
        zmo = stackone_output[:, xbg.shape[1]:, :]  # [B, T, hidden_dim]
        ...
        return xbg, mo_generator.get_final_tokens() # xbg: [B, L], xid: [B, L], xmo: [B, Step, L]

    def interpolate_one_sample(self, bg_tokens, mo_tokens, timesteps):
        """
        bg_tokens: [B, L]
        id_tokens: [B, L]
        mo_tokens: [B, T * L]
        ...
        # Step 1: generate bg/id tokens
        with torch.no_grad():
            _, _, stackone_output = self.model_engine.forward_bgid(bg_tokens, hmo)
        zmo = stackone_output[:, bg_tokens.shape[1]:, :]  # [B, T, hidden_dim]
    ...
    def generate(self, timesteps=None, mode="test", save_path=None, inputs=None,
        ...
        if iterative == 1:
            xbg, xmo_per_step = self.generate_one_sample(bg_pre, mo_base, timesteps)
        else:
        ...
        if mode == "test":
            ...
            for step in range(timesteps):
                ...
                cur_x = self.tokenizer.decode(bg_tokens, mo_tokens)
            ...
            if iterative == 1:
                samples, tokens = self.decode_one_clip(xbg, mo_base, xmo_per_step[:, -1, :])
            else:
                B = self.gen_opt.batch_size
                samples = [[] for b in range(B)]
                for i in range(iterative):
                    # cur_samples: [B, [T]]
                    cur_samples, _ = self.decode_one_clip(record_bg[i], record_mo_base[i],
                                                          record_mo_perstep[i][:, -1, :])
                    if i == 0:
    ...
    def interpolate(self, timesteps=None, mode="test", save_path=None, inputs=None,
        ...
        timesteps = timesteps or self.gen_opt.timesteps
        self.model_engine.eval()
        xmo_per_step = self.interpolate_one_sample(bg_tokens, mo_tokens, timesteps)

        if mode == "sample":
            # Only visualize final step
            samples, tokens = self.decode_one_clip(bg_tokens,
