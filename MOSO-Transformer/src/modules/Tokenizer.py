...
class Tokenizer(nn.Module):
    def __init__(self, vqvae_opt, train_opt, tokenizer_opt, load_vqvae):
        ...
        # All vocabs
        pre_vocabs = self.bg_vocabs + self.id_vocabs + self.mo_vocabs
        self.command_tokens = {
            'MASK': pre_vocabs + 0,
            'SOBG': pre_vocabs + 1,
            'EOBG': pre_vocabs + 2,
        ...
        self.vocab_size = self.bg_vocabs + self.id_vocabs + \
                          self.mo_vocabs + self.command_vocabs

        # general vocabs
        # intorporated codebook: [bg_vocab, id_vocab, mo_vocab, cmd_vovab]
        self.bg_vocab_s = 0
        self.id_vocab_s = self.bg_vocabs
        self.mo_vocab_s = self.bg_vocabs + self.id_vocabs
        self.cmd_vocab_s = self.bg_vocabs + self.id_vocabs + self.mo_vocabs
    ...
    def pad_vocab(self, xbg, xid, xmo):
        ...
        if xid is not None:
            newid = xid.clone()
            newid[newid < self.cmd_vocab_s] += self.id_vocab_s
        if xmo is not None:
            newmo = xmo.clone()
            newmo[newmo < self.cmd_vocab_s] += self.mo_vocab_s
