...
from .Encoder_Content import Encoder_Content
...
class VQVAEModel(nn.Module):
    def __init__(self, model_opt, opt):
        ...
        if model_opt.get('decoder_type', 'default') in ['default', 'decoder_woPA']:
            self._decoder = Decoder(num_hiddens=num_hiddens,
                                    ...
                                    ds_motion=ds_motion,
                                    ds_background=ds_content,
                                    ds_identity=ds_content)
        else:
    ...
    def _decode(self, co_tokens, _, mo_tokens): # bg, id, mo
        '''
            bg_tokens: [B, 1, H, W]
            id_tokens: [B, 1, H, W]
            mo_tokens: [B, T, H, W]
    ...
    def _generater(self, batch, is_training):
        ...
        return {
            ...
            'nll_loss': nll_loss,
            'x_rec': x_rec,
            'quantize_bg': quantize_mo,
            'quantize_id': quantize_mo,
            'quantize_mo': quantize_mo,
            'vq_output_bg': vq_output_mo,
            'vq_output_id': vq_output_mo,
            ...
            'record_logs':{
                ...
                'rec_loss': recon_loss,
                'lpips_loss': p_loss,
                'quant_loss_bg': vq_output_mo['loss'],
