...
class VideoDiffFramesDataset_EXT_FirstKFrames(Dataset):
    def __init__(self, datapath, idspath, used_fps, img_size, num_frames, limit, K, step=None, ret_mode='dict'):
        super().__init__()
        self.K = K
    ...
    def __getitem__(self, index):
        ...
        for i, file in enumerate(files[start + 2: start + 2 + self.num_frames]):
            cur_img = nxt_img
            if i + 1 <= self.K:
...
class VideoDiffFramesDataset_woCP_EXT_FirstKFrames(Dataset):
    def __init__(self, datapath, idspath, used_fps, img_size, num_frames, limit, K, step=None, ret_mode='dict'):
        super().__init__()
        self.K = K
        self.step = step or num_frames
