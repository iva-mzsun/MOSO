...
class VideoDiffFramesDataset_EXT(Dataset):
    def __init__(self, datapath, idspath, used_fps, img_size, num_frames, limit, step=None, ret_mode='dict'):
        super().__init__()
        self.step = step or num_frames
        ...
        for video_id in self.video_ids:
            ...
            if filen < self.num_frames + 2:
                ...
                for start in range(0, filen-2-self.num_frames, self.step):
                    self.items.append([video_id, start])
                # To utilize all frames, count last clip from the end frame to previous ones.
                self.items.append([video_id, filen-2-self.num_frames])
        # self.items = [["FPS30/2011_09_26_drive_0001_sync_image02", 1]] * 100

        logger.info(f"*** {len(self.video_ids)} videos and {len(self.items)} items from datapath {datapath},  "
                    f"img_size: {img_size}, num_frames: {num_frames}, step: {self.step}")
...
class VideoDiffFramesDataset_wRP_EXT(Dataset):
    def __init__(self, datapath, idspath, used_fps, img_size, num_frames, limit, step=None, ret_mode='dict'):
        super().__init__()
        self.step = step or num_frames
    ...
    def __getitem__(self, index):
        ...
        [ret_img, ret_img_bg, ret_img_id, ret_img_mo] =\
            self.random_crop(self.get_batch(video_id, start))
...
class VideoDiffFramesDataset_woCP_EXT(Dataset):
    def __init__(self, datapath, idspath, used_fps, img_size, num_frames, limit, step=None, ret_mode='dict'):
        super().__init__()
        self.step = step or num_frames
