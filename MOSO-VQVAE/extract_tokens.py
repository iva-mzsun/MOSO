...
if __name__ == '__main__':
    ...
    parser.add_argument('--aug_trainset_with_step', type=int, default=-1)
    parser.add_argument('--local_rank', type=int)
    parser.add_argument('--batchsize', type=int, default=None)
    args = parser.parse_args()
    opt = get_opt_from_yaml(args.opt)
    opt['dataset']['batch_size'] = args.batchsize or opt['dataset']['batch_size']
    ...
    if '_wRP' in opt['dataset']['name']:
        opt['dataset']['name'] = opt['dataset']['name'].replace('_wRP', '')
    if '_EXT' not in opt['dataset']['name']:
    ...
    opt['dataset']['ret_mode'] = 'dict'

    if args.aug_trainset_with_step > 0:
        opt['dataset']['step'] = args.aug_trainset_with_step
    ...
    if not args.skip_valid:
        ...
        for step in tqdm(range(tot_iter)):
            ...
            # if inputs['video_id'][0] == 'FPS30/2011_10_03_drive_0027_sync_image03' and inputs['start'][0] == torch.tensor(1920):
            #     st()
            with torch.no_grad():
